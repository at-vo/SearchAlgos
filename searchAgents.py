...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # starting position and empty list of visited corners
    ...
    def isGoalState(self, state):
        ...
        if current in self.corners:
            # if not in current container
            if not current in container:
                container.append(state[0])
            # check if container has covered all four corners
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            if not hitsWall:
                # make list of visited/unvisited corners
                visited = list(state[1])
                current = (nextx, nexty)
                if current in self.corners and current not in visited:
                    # add to visited when we get to a new corner
                    visited.append(current)
                # update state
                successors.append( ( (current,visited),action,1))
...
def cornersHeuristic(state, problem):
    ...
    while not len(unvisited) == 0:
        # find a list of distances to the corner from current
        minDist = float('inf')
        destination = 0
        for corner in unvisited:
            manhat = util.manhattanDistance(current,corner)
            if manhat < minDist:
                minDist = manhat
                destination = corner
        # add min distance to heuristic
        heuristic += minDist
        # then delete the corner from unvisited
        toRemove = destination
        current = toRemove
        unvisited.remove(toRemove)
        # update current to corner to make consistent
    return heuristic
